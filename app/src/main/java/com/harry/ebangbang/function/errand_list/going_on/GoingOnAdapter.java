 ...
 */
public class GoingOnAdapter extends BaseQuickAdapter<ErrandListEntity.DataBean, BaseViewHolder> {

    public GoingOnAdapter(int layoutResId) {
        super(layoutResId);
    }
    ...
    protected void convert(BaseViewHolder helper, ErrandListEntity.DataBean item) {
        helper.setText(R.id.tv_order_number, "订单号:" + item.orderNumber)
                ...
                .addOnClickListener(R.id.tv_call_rider)
                .addOnClickListener(R.id.tv_rider_location);

        if (item.orderStatus == 1) {
            helper.setGone(R.id.tv_rider_location, false)
                    .setGone(R.id.tv_call_rider, false);
        } else {
            helper.setGone(R.id.tv_rider_location, true)
                    .setGone(R.id.tv_call_rider, true);
        }
